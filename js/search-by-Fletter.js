let FletterInput = document.querySelector("#letter-input");
FletterInput.addEventListener("input", function () {
  getMealsByFletter(FletterInput.value);
});

export async function getMealsByFletter(Fletter) {
  try {
    $("#Search")
...
function showCards(mealsCard) {
  ...
  for (let i = 0; i < Math.min(20, mealsCard.length); i++) {
    meals += `
    <div class="col-12 col-sm-6 col-md-3 meal-card" data-id="${mealsCard[i].idMeal}">
                        <figure class="w-100 position-relative rounded-4 overflow-hidden">
                            <img class="w-100 rounded-4" src="${mealsCard[i].strMealThumb}" alt="meal">
